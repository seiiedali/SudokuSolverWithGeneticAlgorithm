...
namespace sudoku
{
    class NineSA
    {
        ...
        {
            Console.WriteLine("working on the solution of 9*9 sudoku table(SA) ... \n");
        ...
        {
            numberPool = new int[length] { length, length, length, length, length, length, length, length, length };
        ...
        private int scoreCalculator()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                for (int j = 0; j < length; j++)
                {
                    tempCheck[j] = neighbor[(i / 3) * 3 + j / 3, i * 3 % 9 + j % 3];
